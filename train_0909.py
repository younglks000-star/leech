...
BASE_DATE = dt.datetime(2013, 1, 1)  # 네 코드에서 쓰던 기준 날짜
...
def build_loaders(args, pred_len):
    ...
    embed_root = os.path.join("./Image_Embeddings_patch", base, f"out{pred_len}")
    ...
    test_loader  = DataLoader(test_set,  batch_size=args.batch_size, shuffle=False, drop_last=False, num_workers=args.num_workers)
    ...
    if isinstance(emb, dict):
        ...
        for k, v in emb.items():
            if torch.is_tensor(v):
                ...
                out[k] = v  # meta 등 텐서가 아닌 것들은 그대로 둠
        return out
    # 프롬프트 임베딩(tensor)인 경우
...
def main():
    ...
    for seq_output in args.output_lens:
        ...
        # 3) Best tracking (네 루틴대로)
        best = [1e5, 1e5, -1e5]  # [mse, mae, corr]
        now = dt.datetime.now()
        model_tag = f"TimeCMA_{seq_output}"  # Metric.save/plot에서 쓸 모델명
        ...
        for epoch in range(args.epochs):
            ...
            for batch in train_loader:
                ...
                # emb = emb.to(device)         # [B, E, C, 1] (collate 후) or [E, C, 1] 단일일 경우 프롬프트 임베딩
                emb = move_emb_to_device(emb, device) # 이미지 임베딩
            ...
            best = Metric.update(now, save=True, model=model, best=best,
                                 metric=[mse, mae, corr],
        ...
        if len(test_set) > 0:
            first_batch_index = int(test_set.indices[0])  # Dataset_Custom에 indices 제공됨
            predict_dates = [
