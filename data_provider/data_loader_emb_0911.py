...
class Dataset_Custom(Dataset):
    """
    CSV로부터 시계열 윈도우를 만들고, 같은 시작 인덱스 s로 저장된 임베딩(HDF5)을 로드한다.
    - 프롬프트 임베딩 포맷:  { 'embeddings': (E, C) 또는 (E, C, 1) }
      -> 반환: torch.FloatTensor (E, C, 1)
    - 이미지 임베딩 포맷:    { 'K': (N, dk), 'V': (N, dv), 'meta': attrs... }
      -> 반환: dict {'K': FloatTensor(N, dk), 'V': FloatTensor(N, dv), 'meta': dict}
    
    ⚠️ 주의: 이미지(K/V) dict가 반환될 때는, 훈련 루프에서 `emb.to(device)`를 직접 호출하면 안 됩니다.
       대신 모델 내부에서 `emb['K'] = emb['K'].to(device)`, `emb['V'] = emb['V'].to(device)`처럼 옮기세요.
    """
    def __init__(self,
        ...
        model_name="gpt2",
        d_llm=768,                        # (프롬프트 임베딩 E 검증용) 이미지 K/V일 때는 미사용
        expect_num_nodes=None,            # CSV 채널 수 체크 (예: 32)
        embed_root=None,                  # 임베딩 루트 폴더 (…/out{pred_len})
        ...
        # 임베딩 루트 기본값: ./Prompt_Embeddings/<csv베이스명>/
    ...
    def _load_embedding_or_dummy(self, s: int):
        """
        s(윈도우 시작) 기준 파일명 매칭 + 임베딩 로딩.
        - 파일이 없을 때:
          - missing_embedding == 'zeros' 이면 (E,C,1) zeros 반환 (프롬프트 경로 호환)
          - 그 외에는 오류
        - 파일이 있을 때:
          - 'embeddings' dataset -> (E,C,1) FloatTensor
          - 'K' & 'V' datasets   -> {'K': FloatTensor(N, dk), 'V': FloatTensor(N, dv), 'meta': dict}
        """
        ...
        with h5py.File(fpath, 'r') as hf:
            ...
            if 'embeddings' in hf:
                ...
                # 기본 형상 체크 (E,C,1)로 맞춰줌
                if ten.shape[0] != E:
                    raise ValueError(f"E mismatch: file {ten.shape[0]} vs expected {E}")
                if ten.shape[1] != C:
                    raise ValueError(f"C mismatch: file {ten.shape[1]} vs expected {C}")
                if ten.shape[2] != 1:
                    ten = ten[..., :1]
                return ten.float()

            # (2) 이미지 임베딩 포맷 (K/V)
            if 'K' in hf and 'V' in hf:
                K = torch.from_numpy(hf['K'][:]).float()  # (N, dk)
                V = torch.from_numpy(hf['V'][:]).float()  # (N, dv)
                meta = {}
                if 'meta' in hf:
                    for k, v in hf['meta'].attrs.items():
                        # h5py attrs를 파이썬 기본형으로 옮김
        ...
        # 포맷을 알아낼 수 없음
        raise KeyError(f"Unknown embedding format in {fpath} (no 'embeddings' nor 'K'/'V').")
    ...
    def __getitem__(self, idx):
        ...
        embeddings = self._load_embedding_or_dummy(s)           # (E,C,1) or dict(K,V,meta)
