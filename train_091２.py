...
# ✅ Dual을 llmcma.py에서 가져오도록 변경
from models.llmcma import Dual
...
def parse_args():
    ...
    p.add_argument("--d_llm", type=int, default=768)  # GPT-2
...
def build_loaders(args, pred_len):
    ...
    # ✅ 프롬프트 임베딩 루트 (필요시 네가 쓰는 폴더명으로 맞춰줘)
    embed_root = os.path.join("./Prompt_Embeddings", base, f"out{pred_len}")
...
def build_model(device, channel, num_nodes, seq_input, pred_len, args):
    # Dual의 시그니처는 유지. (GPT-2 디코더 옵션은 llmcma.Dual 내부 기본값 사용)
    model = Dual(
        device=device,
        channel=channel,
        num_nodes=num_nodes,
...
# -------------------------
# 프롬프트 임베딩(dict) → 모델이 기대하는 텐서로 강제 변환 어댑터
# -------------------------
...
def coerce_image_kv_to_prompt_tensor(emb: dict, d_llm: int, C: int) -> torch.Tensor:
    ...
    반환: [B, d_llm, C, 1]  (모델이 기대하는 프롬프트 임베딩 텐서)
    ...
    for seq_output in args.output_lens:
        ...
        for epoch in range(args.epochs):
            ...
            for batch in train_loader:
                ...
                # 프롬프트 임베딩을 device로 이동 후, dict면 텐서로 강제
