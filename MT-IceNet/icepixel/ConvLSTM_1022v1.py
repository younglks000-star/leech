# baseline_convlstm_spatial.py
# ✅ ConvLSTM 베이스라인 + Metric_node5 지표/시각화 (Spyder 즉시 표시)
...
def load_daily_stack(root, target_hw=IMG_SHAPE):
    ...
    for d, p in recs:
        ...
        a[a >= 2500] = np.nan           # NSIDC fill → NaN
        a = a / 1000.0                  # 0..1로 정규화
...
def make_land_mask(daily_stack):
    ...
    return (valid.sum(axis=0) > 0).astype(np.float32)  # 1=ocean, 0=land
...
def make_single_temporal_dataset(daily_stack, indices, seq_input, 
    ...
    def _slice_single(t):
        ...
        x = tf.expand_dims(x, -1)  # [seq,H,W,1]
...
# 모델 (기본 ConvLSTM)
# =============================================================================
def build_convlstm_baseline(seq_input, H, W, n_out, land_mask=None, lr=LEARNING_RATE, filt=3):
    inp = Input(shape=(seq_input, H, W, 1), name='input')
    x = ConvLSTM2D(
    ...
    c1 = Conv2D(32, filt, activation='relu', padding='same', kernel_initializer='he_normal')(x)
    c1 = Conv2D(32, filt, activation='relu', padding='same', kernel_initializer='he_normal')(c1)
    b1 = BatchNormalization(axis=-1)(c1); skip1 = b1
    p1 = MaxPooling2D(pool_size=(2, 2))(b1)

    c2 = Conv2D(64, filt, activation='relu', padding='same', kernel_initializer='he_normal')(p1)
    c2 = Conv2D(64, filt, activation='relu', padding='same', kernel_initializer='he_normal')(c2)
    b2 = BatchNormalization(axis=-1)(c2)
    p2 = MaxPooling2D(pool_size=(2, 2))(b2)

    u3 = UpSampling2D(size=(2, 2))(p2)
    u3 = Conv2D(32, 2, activation='relu', padding='same', kernel_initializer='he_normal')(u3)
    m3 = concatenate([b2, u3], axis=3)
    c3 = Conv2D(32, filt, activation='relu', padding='same', kernel_initializer='he_normal')(m3)
    c3 = Conv2D(32, filt, activation='relu', padding='same', kernel_initializer='he_normal')(c3)
    b3 = BatchNormalization(axis=-1)(c3)

    u4 = UpSampling2D(size=(2, 2))(b3)
    u4 = Conv2D(16, 2, activation='relu', padding='same', kernel_initializer='he_normal')(u4)
    m4 = concatenate([skip1, u4], axis=3)
    c4 = Conv2D(16, filt, activation='relu', padding='same', kernel_initializer='he_normal')(m4)
    c4 = Conv2D(16, filt, activation='relu', padding='same', kernel_initializer='he_normal')(c4)

    raw_out = Conv2D(n_out, 1, activation='linear')(c4)  # [H,W,C]
    out = Activation('linear', dtype='float32')(raw_out)
    model = Model(inputs=inp, outputs=out)

    if land_mask is not None:
        mask_4d = tf.constant(land_mask.reshape(1, H, W, 1), dtype=tf.float32)
        def masked_mse(y_true, y_pred):
            ...
            return tf.reduce_sum(squared_diff) / n_total_ocean
...
# =============================================================================
# ✅ 평가 + Metric_node5 시각화 (간결: 2개 시각화만)
# =============================================================================
@torch.no_grad()
def evaluate_with_spatial_viz(pred_maps, true_maps, lead_days, 
                              ...
                              tag_time, land_mask):
    """
    Metric_node5의 지표/시각화를 그대로 사용.
    - 반환: [rmse, bacc, ssim, mse, mae, cor]
    """
    N, h, w, C = pred_maps.shape

    # Metric.metric 입력 형태로 변환: [N, C, H*W]
    pred_ts = pred_maps.reshape(N, h*w, C).transpose(0, 2, 1)
    true_ts = true_maps.reshape(N, h*w, C).transpose(0, 2, 1)
    pred_t = torch.from_numpy(pred_ts).float()
    true_t = torch.from_numpy(true_ts).float()

    metric_list = Metric.metric(pred_t, true_t, n_features=h*w)

    # ▶ 시각화: 대시보드 1회 호출 (리드타임 곡선 + 오차 오버레이)
    ...
    Metric.plot_ice_prediction_dashboard(
        ...
        lead_days=vis_days, sample_idx=0
    )

    # (선택) CSV 유지 — 필요 시 주석 처리 가능
    out_dir = f'./STMA_node/{model_name}/models/{model_name}_{C}_{tag_time.month}{tag_time.day}{tag_time.hour}{tag_time.minute}'
    os.makedirs(out_dir, exist_ok=True)
    predict_dates = [(base_date + timedelta(days=int(first_batch_index + seq_input + int(L)))).strftime("%Y-%m-%d") for L in lead_days]
    pred_first = pred_ts[0]; true_first = true_ts[0]
...
def main():
    print("="*70)
    print("베이스라인: ConvLSTM + Metric_node5 지표/시각화")
    print("="*70)
    ...
    for seq_output in output_lens:
        print(f"\n{'='*70}")
        print(f"실험: +{seq_output}일 연속 예측 (1~{seq_output}일)")
        print(f"{'='*70}")
        ...
        model = build_convlstm_baseline(seq_input, H_eff, W_eff, n_out=seq_output, land_mask=land_mask_d)
        ...
        for epoch in range(Epoch):
            _ = model.fit(train_ds, validation_data=val_ds, epochs=1, verbose=2)
            ...
            best = Metric.update(now, save, model, best, metric, f"{model_name}_{seq_output}", seq_output, epoch)
    ...
    print("\n" + "="*70)
    print("최종 결과 요약 - ConvLSTM Baseline (Metric_node5)")
    print("="*70)
    for k, v in results.items():
        print(f"[+{k}일] RMSE={v['b_rmse']:.4f} | BACC={v['b_bacc']:.4f} | SSIM={v['b_ssim']:.4f} "
              f"| MSE={v['b_mse']:.4f} | MAE={v['b_mae']:.4f} | COR={v['b_cor']:.4f}")
    print("="*70)
