# mt_icenet_itx_style_loop_multi.py
...
# 여기 output_lens 을 너가 실험하고 싶은 예측 길이들로 설정
output_lens   = [7, 14, 21, 28]   # 예: 7일, 14일, 21일, 28일 예측
seq_input     = 30                # 입력 길이
BATCH_SIZE    = 4
Epoch         = 50
LEARNING_RATE = 1e-4
...
def load_daily_stack(root, target_hw=IMG_SHAPE):
    ...
    # 스케일 조정: 0~1 범위로 (원래 0~2540 → 농도 = 값/25.4 → 0~100), 여기선 /100
...
def make_dataset(daily_stack, indices, t_days, lead_days, batch_size=1, shuffle=False, seed=42, downsample=1):
    ...
    def _slice_one(t):
        # 입력 시계열
        x = ds_x[t - t_days : t]  # [t_days, H, W]
        if new_hw is not None:
            x = _maybe_downsample_3d(x, new_hw)
        x = tf.expand_dims(x, -1)  # [t_days, h, w, 1]

        # 타깃: 매일 예측 lead_days 개수만큼 쌓기
...
def evaluate_for_metric_node(pred_maps, true_maps, lead_days, first_batch_index, seq_input, model_name, tag_time):
    ...
    for r in range(topK):
        df = pd.DataFrame({"Date": predict_dates,
