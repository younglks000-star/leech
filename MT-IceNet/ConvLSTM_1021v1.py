...
def evaluate_with_spatial_viz(pred_maps, true_maps, lead_days, 
    ...
    ✅ 평가 + 공간 시각화
    - 기존 Metric 계산
    - 공간 지도 시각화 (Metric_node2 함수 사용)
    """
    N, h, w, C = pred_maps.shape
    
    # [N, h*w, C] -> [N, C, h*w] (기존 Metric용)
    ...
    # ✅ 공간 시각화 추가 (Metric_node2의 새 함수들)
    # 1. 주요 lead days 비교 (Day 1, 7, 14 or 최대)
    viz_days = []
    ...
    Metric.plot_spatial_comparison(
        ...
        lead_days=viz_days,
        sample_idx=0  # 첫 번째 테스트 샘플
    )
    
    # 2. 시간별 진화 (Day 1~7 or 전체)
