import torch
import torch.nn as nn
import torch.nn.functional as F
from layers.StandardNorm import Normalize
from layers.Cross_Modal_Align import CrossModal


class Dual(nn.Module):
    """
    TimeCMA (Temporal → Spatial), 0924v2 기반 + 옵션 3종
    - [opt] Multi-horizon head (학습 멀티, 평가 단일 h 또는 dict)
    - [opt] Multi-scale decoding (low-pass depthwise conv branch)
    - [opt] Time–Frequency hybrid cross (q_s에 rFFT Top-K 통과)

    기본은 0924v2와 동일 동작(옵션 OFF).
    """

    def __init__(
        self,
        device="cuda:7",
        channel=32,          # C: 노드 임베딩 채널 수
        num_nodes=7,         # N: 노드/클러스터 개수
        seq_len=96,          # L_in
        pred_len=96,         # L_out (단일헤드 모드에서 사용)
        dropout_n=0.1,
        d_llm=768,           # 프롬프트 임베딩 피처 차원
        e_layer=1,
        d_layer=1,
        d_ff=32,
        head=8,

        # ===== 0924v2 기본 옵션 =====
        use_time_feat=True,
        time_fourier_periods=(7.0, 30.4375, 91.3125, 365.25),
        p_drop_prompt=0.15,
        p_drop_image=0.15,

        # ===== (1) 멀티-호라이즌 헤드 =====
        multi_head=False,                 # 기본 False → 0924v2와 동일 출력
        pred_set=(180, 360, 720, 1080, 1440, 1800),
        return_dict=True,                 # True면 dict 반환, False면 target_horizon만 반환

        # ===== (2) 다중 스케일 디코딩(저주파 브랜치) =====
        use_ms_decoder=False,             # 기본 OFF
        ms_kernel=9,                      # time 방향 depthwise conv kernel (홀수 권장)
        ms_gain_init=0.10,                # 저주파 브랜치 가중 게이트 초기값

        # ===== (3) 시간-주파수 하이브리드 Cross =====
        use_tf_hybrid=False,              # 기본 OFF
        tf_topk_ratio=0.25,               # 토큰축 주파수 상위 K(=저주파 영역)만 유지 (0~1)
        tf_gain_init=0.10,                # 필터 residual 주입 게이트 초기값
    ):
        super().__init__()

        # --------- 하이퍼 저장 ----------
        self.device = device
        self.channel = channel
        self.num_nodes = num_nodes
        self.seq_len = seq_len
        self.pred_len = pred_len
        self.dropout_n = dropout_n
        self.d_llm = d_llm
        self.e_layer = e_layer
        self.d_layer = d_layer
        self.d_ff = d_ff
        self.head = head

        self.use_time_feat = use_time_feat
        self.time_fourier_periods = time_fourier_periods
        self.p_drop_prompt = float(p_drop_prompt)
        self.p_drop_image = float(p_drop_image)

        self.multi_head = bool(multi_head)
        self.pred_set = list(pred_set)
        self.return_dict = bool(return_dict)

        self.use_ms_decoder = bool(use_ms_decoder)
        self.ms_kernel = int(ms_kernel if ms_kernel % 2 == 1 else ms_kernel + 1)
        self.ms_gain = nn.Parameter(torch.tensor(float(ms_gain_init)))

        self.use_tf_hybrid = bool(use_tf_hybrid)
        self.tf_topk_ratio = float(tf_topk_ratio)
        self.tf_gain = nn.Parameter(torch.tensor(float(tf_gain_init)))

        # --------- RevIN ----------
        self.normalize_layers = Normalize(self.num_nodes, affine=False).to(self.device)

        # --------- Fourier feat ----------
        if self.use_time_feat:
            self.time_ff = nn.Sequential(
                nn.Linear(2 * len(self.time_fourier_periods), self.num_nodes, bias=False),
                nn.GELU()
            ).to(self.device)
            nn.init.xavier_uniform_(self.time_ff[0].weight)

        # --------- [B,N,L] → [B,N,C] ----------
        self.length_to_feature = nn.Linear(self.seq_len, self.channel).to(self.device)
        self.ln_after_len2feat = nn.LayerNorm(self.channel).to(self.device)

        # --------- TS Encoder ----------
        self.ts_encoder_layer = nn.TransformerEncoderLayer(
            d_model=self.channel, nhead=self.head, batch_first=True,
            norm_first=True, dropout=self.dropout_n
        ).to(self.device)
        self.ts_encoder = nn.TransformerEncoder(
            self.ts_encoder_layer, num_layers=self.e_layer
        ).to(self.device)

        # --------- Prompt Encoder ----------
        self.prompt_encoder_layer = nn.TransformerEncoderLayer(
            d_model=self.d_llm, nhead=self.head, batch_first=True,
            norm_first=True, dropout=self.dropout_n
        ).to(self.device)
        self.prompt_encoder = nn.TransformerEncoder(
            self.prompt_encoder_layer, num_layers=self.e_layer
        ).to(self.device)
        self.prompt_pool = nn.AdaptiveAvgPool1d(self.d_llm)
        self.ln_prompt_in = nn.LayerNorm(self.d_llm).to(self.device)

        # --------- Temporal Cross (Prompt) ----------
        self.cross = CrossModal(
            d_model=self.num_nodes, n_heads=1, d_ff=self.d_ff, norm='LayerNorm',
            attn_dropout=self.dropout_n, dropout=self.dropout_n, pre_norm=True,
            activation="gelu", res_attention=True, n_layers=1, store_attn=False
        ).to(self.device)

        # --------- Spatial Cross (Image) ----------
        self.c_to_nodes = nn.Linear(self.channel, self.num_nodes, bias=False).to(self.device)
        self.nodes_to_c = nn.Linear(self.num_nodes, self.channel, bias=False).to(self.device)
        self.ln_q_spatial_in = nn.LayerNorm(self.num_nodes).to(self.device)

        self.img_k_proj = nn.Linear(self.num_nodes, self.num_nodes, bias=False).to(self.device)
        self.img_v_proj = nn.Linear(self.num_nodes, self.num_nodes, bias=False).to(self.device)
        self.ln_img_kv = nn.LayerNorm(self.num_nodes).to(self.device)

        self.cross_spatial = CrossModal(
            d_model=self.num_nodes, n_heads=1, d_ff=self.d_ff, norm='LayerNorm',
            attn_dropout=self.dropout_n, dropout=self.dropout_n, pre_norm=True,
            activation="gelu", res_attention=True, n_layers=1, store_attn=False
        ).to(self.device)

        # --------- Residual 게이트 ----------
        self.g_t = nn.Parameter(torch.tensor(0.1))
        self.g_s = nn.Parameter(torch.tensor(0.1))

        # --------- Decoder ----------
        self.decoder_layer = nn.TransformerDecoderLayer(
            d_model=self.channel, nhead=self.head, batch_first=True,
            norm_first=True, dropout=self.dropout_n
        ).to(self.device)
        self.decoder = nn.TransformerDecoder(
            self.decoder_layer, num_layers=self.d_layer
        ).to(self.device)

        self.c_to_length = nn.Linear(self.channel, self.pred_len, bias=True).to(self.device)
        self.ln_after_spatial = nn.LayerNorm(self.channel).to(self.device)

        # ===== 멀티-호라이즌 헤드 =====
        if self.multi_head:
            self.heads = nn.ModuleDict({
                str(h): nn.Linear(self.channel, h, bias=True).to(self.device)
                for h in self.pred_set
            })

        # ===== MS 디코더(저주파 브랜치) =====
        if self.use_ms_decoder:
            pad = self.ms_kernel // 2
            self.ms_conv = nn.Conv1d(
                in_channels=self.num_nodes, out_channels=self.num_nodes,
                kernel_size=self.ms_kernel, padding=pad, groups=self.num_nodes, bias=True
            ).to(self.device)

    # ---------- utils ----------
    def param_num(self):
        return sum(p.numel() for p in self.parameters())

    def count_trainable_params(self):
        return sum(p.numel() for p in self.parameters() if p.requires_grad)

    @staticmethod
    def _ensure_prompt_shape(p: torch.Tensor) -> torch.Tensor:
        return p.squeeze(-1) if (p is not None and p.dim() == 4) else p

    @staticmethod
    def _build_time_fourier(L: int, device, dtype, periods):
        # [L, 2K]
        t = torch.arange(L, device=device, dtype=dtype)
        feats = []
        for p in periods:
            w = 2.0 * torch.pi * t / p
            feats.extend([torch.sin(w), torch.cos(w)])
        return torch.stack(feats, dim=1).transpose(0, 1).transpose(0, 1)  # [L,2K]

    def _prep_image_kv(self, emb_image, B, N, device):
        assert isinstance(emb_image, dict) and ('K' in emb_image) and ('V' in emb_image), \
            "emb_image must be dict with keys 'K' and 'V'"
        K, V = emb_image['K'], emb_image['V']
        if not torch.is_tensor(K): K = torch.as_tensor(K)
        if not torch.is_tensor(V): V = torch.as_tensor(V)
        K = K.to(device).float()
        V = V.to(device).float()
        if K.dim() == 2: K = K.unsqueeze(0).expand(B, -1, -1)  # [B,N,d]
        if V.dim() == 2: V = V.unsqueeze(0).expand(B, -1, -1)
        assert K.size(1) == N and V.size(1) == N, f"K/V must have N={N} at dim=1, got {K.size()} / {V.size()}"
        K = K.transpose(1, 2).contiguous()  # [B, d, N]
        V = V.transpose(1, 2).contiguous()
        return K, V

    # --- 시간-주파수 하이브리드: 토큰축(N) rFFT → Top-K 유지 → iFFT ---
    def _tf_filter_tokens(self, q_s: torch.Tensor) -> torch.Tensor:
        # q_s: [B, N, N] (토큰=N, feature=N)
        if not self.use_tf_hybrid or self.tf_topk_ratio <= 0.0:
            return q_s
        B, T, F = q_s.shape
        # 토큰축(=dim=1) rFFT
        spec = torch.fft.rfft(q_s, dim=1)                    # [B, T_r, F]
        T_r = spec.size(1)
        k = max(1, int(T_r * self.tf_topk_ratio))            # 저주파 상위 K만
        # high-freq zeroing
        spec[:, k:, :] = 0.0
        # iFFT 복원
        filt = torch.fft.irfft(spec, n=T, dim=1)             # [B, T, F]
        # residual gate
        return q_s + torch.sigmoid(self.tf_gain) * (filt - q_s)

    def gate_values(self):
        return float(F.softplus(self.g_t).detach().cpu()), float(F.softplus(self.g_s).detach().cpu())

    # ---------- forward ----------
    def forward(self, input_data, input_data_mark, emb_prompt, emb_image, target_horizon=None):
        """
        input_data : [B, L, N]
        emb_prompt : [B, E, N] or [B, E, N, 1]
        emb_image  : {'K':[B,N,dk]|[N,dk], 'V':[B,N,dv]|[N,dv]}

        Returns
        -------
        - multi_head=False                     → [B, L_out, N]
        - multi_head=True, return_dict=True    → dict {h: [B,h,N]}
        - multi_head=True, return_dict=False   → [B, target_horizon, N]
        """
        x = input_data.float()                  # [B, L, N]
        p = self._ensure_prompt_shape(emb_prompt).float() if emb_prompt is not None else None
        B, L, N = x.size()
        assert N == self.num_nodes, f"N mismatch: input {N} vs model {self.num_nodes}"

        # RevIN
        x = self.normalize_layers(x, 'norm')    # [B, L, N]

        # Fourier (입력 길이 L과 정합)
        if self.use_time_feat:
            tf = self._build_time_fourier(L, device=x.device, dtype=x.dtype, periods=self.time_fourier_periods)  # [L,2K]
            tf_proj = self.time_ff(tf)          # [L, N]
            x = x + tf_proj.unsqueeze(0)        # [B, L, N]

        # [B,L,N] -> [B,N,L] -> L→C
        x = x.permute(0, 2, 1)                  # [B, N, L]
        x = self.length_to_feature(x)           # [B, N, C]
        x = self.ln_after_len2feat(x)           # 안정화

        # Prompt 인코딩
        if p is not None:
            p = p.permute(0, 2, 1)              # [B, N, E]
            p = self.prompt_pool(p)             # [B, N, d_llm]
            p = self.ln_prompt_in(p)            # LN
            p_enc = self.prompt_encoder(p)      # [B, N, d_llm]
            p_enc = p_enc.permute(0, 2, 1)      # [B, d_llm, N]
        else:
            p_enc = None

        if self.training and (p_enc is not None) and (torch.rand(1).item() < self.p_drop_prompt):
            p_enc = None

        # TS encoder
        enc_nodes = self.ts_encoder(x)          # [B, N, C]
        q_t = enc_nodes.permute(0, 2, 1)        # [B, C, N]

        # (1) Temporal Cross (Prompt)
        if p_enc is not None:
            delta_t = self.cross(q_t, p_enc, p_enc)  # [B, C, N]
            z_t = q_t + F.softplus(self.g_t) * delta_t
        else:
            z_t = q_t
        z_t = z_t.permute(0, 2, 1)              # [B, N, C]

        # (2) Spatial Cross (Image)
        q_s = self.c_to_nodes(z_t)              # [B, N, N]
        q_s = self.ln_q_spatial_in(q_s)         # 안정화

        k_img, v_img = self._prep_image_kv(emb_image, B=B, N=self.num_nodes, device=z_t.device)  # [B, d, N]
        k_img = self.img_k_proj(k_img)          # [B, d, N]
        v_img = self.img_v_proj(v_img)          # [B, d, N]
        k_img = self.ln_img_kv(k_img)
        v_img = self.ln_img_kv(v_img)

        if self.training and (torch.rand(1).item() < self.p_drop_image):
            k_img, v_img = None, None

        if (k_img is not None) and (v_img is not None):
            # [opt] 토큰축 저주파만 통과(시간-주파수 하이브리드)
            q_s_for_attn = self._tf_filter_tokens(q_s) if self.use_tf_hybrid else q_s
            delta_s = self.cross_spatial(q_s_for_attn, k_img, v_img)  # [B, N, N]
            z_s = q_s + F.softplus(self.g_s) * delta_s
        else:
            z_s = q_s

        z = self.nodes_to_c(z_s)                # [B, N, C]
        z = self.ln_after_spatial(z)

        # Decoder 공통 은 동일
        dec_hid = self.decoder(z, z)            # [B, N, C]

        # ===== 출력 경로 =====
        if self.multi_head:
            outs = {}
            for h in self.pred_set:
                y = self._decode_to_len(dec_hid, h)  # [B, L=h, N]
                outs[h] = self.normalize_layers(y, 'denorm')
            if self.return_dict:
                return outs
            else:
                assert target_horizon is not None, "return_dict=False면 target_horizon을 지정해야 합니다."
                return outs[target_horizon]
        else:
            y = self._decode_to_len(dec_hid, self.pred_len)   # [B, L_out, N]
            y = self.normalize_layers(y, 'denorm')
            return y

    # ---- length decoding with optional multi-scale low-pass branch ----
    def _decode_to_len(self, dec_hid: torch.Tensor, out_len: int) -> torch.Tensor:
        """
        dec_hid : [B, N, C]
        return  : [B, out_len, N]
        """
        # 기본 경로(아이덴티티)
        y_id = self.c_to_length(dec_hid)        # [B, N, L]
        y_id = y_id.permute(0, 2, 1)            # [B, L, N]

        if not self.use_ms_decoder:
            return y_id

        # 저주파 브랜치 (depthwise conv over time)
        y_lp = y_id.permute(0, 2, 1)            # [B, N, L]
        y_lp = self.ms_conv(y_lp)               # [B, N, L]
        y_lp = y_lp.permute(0, 2, 1)            # [B, L, N]

        # 가중 합산 (게이트는 sigmoid로 0~1)
        return y_id + torch.sigmoid(self.ms_gain) * y_lp
