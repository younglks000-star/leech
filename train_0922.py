...
#
# TimeCMA 스타일: Temporal 단계는 Q=시계열, K/V=프롬프트 임베딩,
#                Spatial 단계는 Q=Temporal 출력, K/V=이미지 임베딩으로 사용.
# 전제:
#   - data_provider.data_loader_emb_0911.Dataset_Custom 이
#       return x, y, x_mark, y_mark, emb_prompt, emb_image
#     형태로 반환 (emb_prompt: [B,E,C,1], emb_image: {'K':[B,C,Dimg], 'V':[B,C,Dimg], ...})
#   - models.TimeCMA.Dual 이 forward(x, x_mark, emb_prompt, emb_image) 시그니처로 동작
#     (내부에서 Temporal Cross(KV=prompt) -> Spatial Cross(KV=image))
#
# 경로 템플릿:
#   prompt_root_base/<csv_base>/out{pred_len}/{train|val|test}/{s}.h5  # 'embeddings' 포함
#   image_root_base/<csv_base>/out{pred_len}/{train|val|test}/{s}.h5   # 'Kc'/'Vc' (또는 'K'/'V') 포함
...
OVERRIDE = {
    ...
    # ✅ 새로 추가: 임베딩 루트 베이스(원하면 절대경로로 바꿔도 됨)
    "prompt_root_base": r".\Prompt_Embeddings_llama360",
    "image_root_base":  r".\Image_Embeddings_cluster_768",
}
...
def parse_args():
    """
    - OVERRIDE.enable=True면 OVERRIDE 값을 argparse 기본값으로 설정
    - CLI에서 넘긴 인자만 해당 항목을 덮어씀
    - output_lens는 str("180,360")/list 모두 허용하고 최종적으로 list[int]로 정규화
    """
    ...
    # 콤마 구분 문자열/리스트 모두 허용 (정규화는 아래서 처리)
    p.add_argument("--output_lens")
    # ✅ 새 인자
    p.add_argument("--prompt_root_base", type=str)
    p.add_argument("--image_root_base", type=str)
...
def build_loaders(args, pred_len):
    ...
    - prompt/image 임베딩 루트를 각각 out{pred_len}에 맞춰 전달
    """
    base = os.path.splitext(os.path.basename(args.csv_path))[0]

    prompt_root = os.path.join(args.prompt_root_base, base, f"out{pred_len}")
    image_root  = os.path.join(args.image_root_base,  base, f"out{pred_len}")
    ...
    ds_kwargs = dict(
        ...
        # ✅ 두 임베딩 루트
        prompt_embed_root=prompt_root,
        image_embed_root=image_root,
    ...
    for seq_output in args.output_lens:
        ...
        for epoch in range(args.epochs):
            ...
            for batch in train_loader:
                ...
                # emb_prompt: [B, E, C, 1]
                # emb_image:  {'K':[B,C,Dimg], 'V':[B,C,Dimg], ...}
                emb_prompt = move_to_device(emb_prompt, device)
                emb_image  = move_to_device(emb_image,  device)

                optimizer.zero_grad()
                out = model(x, x_mark, emb_prompt=emb_prompt, emb_image=emb_image)  # [B, O, C]
            ...
            with torch.no_grad():
                for batch in test_loader:
                    ...
                    pred = model(x, x_mark, emb_prompt=emb_prompt, emb_image=emb_image)
